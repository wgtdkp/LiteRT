...
load("//tflite:special_rules.bzl", "flex_portable_tensorflow_deps")
...
def generate_flex_kernel_header(
    ...
    list_ops_tool = clean_dep("//tflite/tools:list_flex_ops_main")
    if additional_deps:
        tf_cc_binary(
            name = "%s_list_flex_ops_main" % name,
            deps = [
                clean_dep("//tflite/tools:list_flex_ops_main_lib"),
            ] + additional_deps,
            testonly = testonly,
        )
    ...
    tool = clean_dep("@org_tensorflow//tensorflow/python/tools:print_selective_registration_header")
    native.genrule(
...
def tflite_flex_cc_library(
    ...
    """
    portable_tensorflow_lib = clean_dep("@org_tensorflow//tensorflow/core:portable_tensorflow_lib")
    if models:
        ...
        cc_library(
            ...
            srcs = if_mobile([
                clean_dep("@org_tensorflow//tensorflow/core:portable_op_registrations_and_gradients"),
                clean_dep("@org_tensorflow//tensorflow/core/kernels:portable_core_ops"),
                clean_dep("@org_tensorflow//tensorflow/core/kernels:portable_extended_ops"),
            ...
            textual_hdrs = [
                clean_dep("@org_tensorflow//tensorflow/core/kernels:portable_all_ops_textual_hdrs"),
            ],
            visibility = visibility,
            deps = flex_portable_tensorflow_deps() + [
                clean_dep("@ducc//:fft_wrapper"),
                clean_dep("@org_tensorflow//tensorflow/core:protos_all_cc"),
                clean_dep("@org_tensorflow//tensorflow/core:portable_tensorflow_lib_lite"),
                clean_dep("@org_tensorflow//tensorflow/core/platform:strong_hash"),
                clean_dep("//tflite/delegates/flex:portable_images_lib"),
            ],
            alwayslink = 1,
            testonly = testonly,
        )
    ...
    delegate_symbol = []
    if link_symbol:
        delegate_symbol.append(clean_dep("//tflite/delegates/flex:delegate_symbol"))

    # Define a custom flex delegate with above tensorflow_lib.
    cc_library(
        name = name,
        hdrs = [
            clean_dep("//tflite/delegates/flex:delegate.h"),
        ],
        compatible_with = compatible_with,
        visibility = visibility,
        deps = [
            clean_dep("//tflite/delegates/flex:delegate_data"),
            clean_dep("//tflite/delegates/flex:delegate_only_runtime"),
            clean_dep("//tflite/delegates/utils:simple_delegate"),
        ] + select({
            ...
            clean_dep("@org_tensorflow//tensorflow:chromiumos"): [
                portable_tensorflow_lib,
            ],
            "//conditions:default": [
                clean_dep("@org_tensorflow//tensorflow/core:tensorflow"),
                clean_dep("//tflite/core/c:private_common"),
            ],
    ...
    tflite_cc_shared_object(
        ...
        linkopts = select({
            "@org_tensorflow//tensorflow:macos": [
                "-Wl,-exported_symbols_list,$(location //tflite/delegates/flex:exported_symbols.lds)",
            ],
            "@org_tensorflow//tensorflow:windows": [],
            "//conditions:default": [
                "-Wl,-z,defs",
                "-Wl,--version-script,$(location //tflite/delegates/flex:version_script.lds)",
            ],
        }),
        per_os_targets = True,
        deps = [
            "%s_flex_delegate" % name,
            "//tflite/delegates/flex:exported_symbols.lds",
            "//tflite/delegates/flex:version_script.lds",
        ],
    )
...
def tflite_flex_jni_library(
    ...
    cc_library(
        ...
        srcs = [
            clean_dep("//tflite/testing:init_tensorflow.h"),
            clean_dep("//tflite/testing:init_tensorflow.cc"),
            clean_dep("//tflite/delegates/flex/java/src/main/native:flex_delegate_jni.cc"),
        ],
        copts = tflite_copts(),
        testonly = testonly,
        visibility = visibility,
...
def tflite_flex_android_library(
    ...
    android_library(
        name = name,
        srcs = [clean_dep("//tflite/delegates/flex/java/src/main/java/org/tensorflow/lite/flex:flex_delegate")],
        manifest = clean_dep("//tflite/java:AndroidManifest.xml"),
        proguard_specs = [clean_dep("//tflite/java:proguard.flags")],
        custom_package = custom_package,
        testonly = testonly,
        deps = [
            ":%s_native" % name,
            clean_dep("//tflite/java:tensorflowlite_java"),
