...
#ifndef THIRD_PARTY_ODML_LITERT_TENSOR_INTERNAL_TYPE_ID_H_
#define THIRD_PARTY_ODML_LITERT_TENSOR_INTERNAL_TYPE_ID_H_
...
constexpr absl::string_view::size_type rfind(absl::string_view str,
                                             absl::string_view substr) {
  const absl::string_view::size_type len = substr.length();
  for (absl::string_view::size_type pos = str.length() - len; pos > 0; --pos) {
    if (str.substr(pos, len) == substr) {
      return pos;
    }
  }
  return str.substr(0, len) == substr ? 0 : absl::string_view::npos;
}
...
constexpr absl::string_view ExtractTypeName() {
  ...
  constexpr absl::string_view suffix = "]";
  ...
  constexpr absl::string_view suffix = ">(void)";
#else
#error "Unsupported compiler: function signature macro not available."
  ...
  constexpr absl::string_view::size_type finish = rfind(name, suffix);
...
[[maybe_unused]] constexpr bool IsIdentifierChar(const char c) {
...
#endif  // THIRD_PARTY_ODML_LITERT_TENSOR_INTERNAL_TYPE_ID_H_
